...
class TorchMMVAE(nn.Module):
    ...
    def __init__(self, vaes:list, n_latents: int, obj: str, beta=1, K=1):
        ...
        self._pz_params = nn.ParameterList([
            ...
            nn.Parameter(torch.zeros(1, self.n_latents), requires_grad=False)  # logvar
        ])
    ...
    def get_crossmodal_combinations(self, x):
        cross_combos = list(itertools.product(list(x.keys()), repeat=2))
        cross = []
        for l in cross_combos:
            if l[0] != l[1]:
                cross.append(list(l))
        return cross
    ...
    def decode(self, samples):
        ...
        for modality, vae in self.vaes.items():
            if modality in samples and not samples[modality]["latents"] is None:
                pz_x = vae.decode(samples[modality])
                pz_xs[modality] = pz_x
