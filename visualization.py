...
def t_sne_ploting(df, data, path, labels, mod_names, K=1):
    ...
    if labels:
        ...
        df["y"] = [x.replace("drag", "move").replace("shift", "move").replace("raise", "lift").replace("hoist", "lift") for x in labels]
...
def tensors_to_df(tensors, head=None, keys=None, ax_names=None):
    """Taken from https://github.com/iffsid/mmvae"""
    dfs = [tensor_to_df(tensor.unsqueeze(-1), ax_names=ax_names) for tensor in tensors]
